...
export class PropertyService {
  constructor(
    ...
    private readonly propertyRep: Repository<Property>,
  ...
  async findByAdmin(adminId: string): Promise<Property[]> {
    ...
    const filteredProperties = properties.filter((property) =>
      property.admins.includes(adminId),
    );
  ...
  async addAdmins(id: string, adminId: string): Promise<Property> {
    ...
    if (!adminId) {
      throw new HttpException(
        'ID do administrador não fornecido',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (isAlreadyInserted) {
      throw new HttpException(
        'Administrador já cadastrado na propriedade',
        HttpStatus.BAD_REQUEST,
      );
    }
  ...
  async removeAdmin(id: string, adminId: string): Promise<Property> {
    ...
    if (index === -1) {
      throw new HttpException(
        'Administrador não associado à propriedade',
        HttpStatus.BAD_REQUEST,
      );
    }
