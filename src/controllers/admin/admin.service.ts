...
export class AdminService {
  constructor(
    ...
    @Inject(forwardRef(() => ClinicalPermissionsService))
    private readonly clinicalService: ClinicalPermissionsService,
    @Inject(forwardRef(() => CommercialPermissionsService))
    private readonly commercialService: CommercialPermissionsService,
    @Inject(forwardRef(() => FinancialPermissionsService))
    private readonly financialService: FinancialPermissionsService,
    @Inject(forwardRef(() => PurchasesPermissionsService))
    private readonly purchasesService: PurchasesPermissionsService,
    @Inject(forwardRef(() => RegistrationPermissionsService))
    private readonly registrationService: RegistrationPermissionsService,
    @Inject(forwardRef(() => ReproductivePermissionsService))
    private readonly reproductiveService: ReproductivePermissionsService,
    @Inject(forwardRef(() => SportPermissionsService))
    private readonly sportService: SportPermissionsService,
    @Inject(forwardRef(() => TrackingPermissionsService))
    private readonly trackingService: TrackingPermissionsService,
  ...
  async findOne(id: string): Promise<any> {
    ...
    const commercial = await this.commercialService.findByUsername(
      verify.username,
    );
    const financial = await this.financialService.findByUsername(
      verify.username,
    );
    const purchases = await this.purchasesService.findByUsername(
      verify.username,
    );
    const registration = await this.registrationService.findByUsername(
      verify.username,
    );
    const reproductive = await this.reproductiveService.findByUsername(
      verify.username,
    );
