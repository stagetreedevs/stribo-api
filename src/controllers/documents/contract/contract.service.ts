...
export class ContractService {
  ...
  async create(body: any): Promise<any> {
    ...
    } else if (!Array.isArray(installments) || installments.length < 2) {
      throw new HttpException(
        'Pagamento parcelado deve conter no mínimo 2 parcelas.',
        HttpStatus.BAD_REQUEST,
      );
    }

    const newContract = await this.contractRepository.save(body);
    newContract.pdf_url = await this.generatePdf(newContract);
    await this.contractRepository.update({ id: newContract.id }, newContract);

    const contractDate = new Date(newContract.date);
    ...
    const sendDateUTC = this.getNotificationDateTimeUTC(contractDate, 2, 12, 0);
  ...
  async findFiltered(
    ...
    if (property) {
      queryBuilder.andWhere('contract.property = :property', {
        property: property,
      });
    }

    if (body.provider) {
      queryBuilder.andWhere('contract.provider = :provider', {
        provider: body.provider,
      });
    }

    if (body.animal_name) {
      queryBuilder.andWhere('contract.animal_name = :animal_name', {
  ...
  async generatePdf(contrato: any): Promise<string> {
    ...
    const pdfBuffer: Buffer = await new Promise((resolve) => {
      ...
      doc.image(join(process.cwd(), 'src/assets/6af3b4.png'), imageX, 15, {
        width: 70,
        height: 55,
        align: 'center',
      });
      ...
      doc
        .font('Helvetica-Bold')
        .fontSize(14)
        .text(`${contrato.title}`, { align: 'center' });
      doc.moveDown();
      ...
      doc
        .font('Helvetica-Bold')
        .fontSize(12)
        .text('Valor e Termos de Pagamento:');
      doc
        .font('Helvetica')
        .fontSize(12)
        .text(`Valor de Pagamento: R$ ${contrato.value}`);
      if (contrato.payment) {
      ...
      doc
        .font('Helvetica')
        .fontSize(12)
        .text(`Status do Contrato: ${contrato.status}`);
      doc.moveDown();
      ...
      doc
        ...
        .fontSize(12)
        .text(
          '1. Qualquer alteração neste contrato deve ser feita por escrito e assinada por ambas as partes.',
          { align: 'justify' },
        );
      doc
        .font('Helvetica')
        .fontSize(12)
        .text(
          '2. Este contrato é regido pelas leis na República Federativa do Brasil.',
          { align: 'justify' },
        );
      doc
        .font('Helvetica')
        .fontSize(12)
        .text(
          '3. Em caso de litígio, as partes concordam em tentar resolver o problema de forma amigável. Se isso não for possível, as partes concordam com a jurisdição exclusiva dos tribunais competentes na República Federativa do Brasil.',
          { align: 'justify' },
        );
      doc.moveDown();
      ...
      doc
        .font('Helvetica-Bold')
        .fontSize(12)
        .text('Vendedor: _____________________________');
      doc.moveDown();
      doc
        .font('Helvetica-Bold')
        .fontSize(12)
        .text('CPF: _____________________________');
      doc.moveDown();
      doc
        .font('Helvetica-Bold')
        .fontSize(12)
        .text('Comprador: _____________________________');
      doc.moveDown();
      doc
        .font('Helvetica-Bold')
        .fontSize(12)
      ...
      doc
        .font('Helvetica-Bold')
        .fontSize(12)
        .text('Data: ____________________________');
      doc.moveDown();
    ...
    const url = await this.s3Service.uploadS3(
      pdfBuffer,
      path,
      `${contrato.title}.pdf`,
    );
