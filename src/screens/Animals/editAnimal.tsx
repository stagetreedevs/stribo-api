    ...
    try {
      if (state) {
        ...
        Object.entries(form).forEach(entry => {
          ...
          if (key === 'castrationDate') {
            return formData.append(
              key,
              new Date(form.castrationDate).toISOString(),
            );
          }
          if (key === 'birthDate') {
            return formData.append(key, new Date(form.birthDate).toISOString());
          }
      ...
      Toast.show({
        type: 'error',
        text1: 'Não foi possivel efetuar a edição',
